package com.myspring.pro12.ex03;
...
public class BoardController {
	...
	public ResponseEntity<ArticleVO> findArticle (@PathVariable("articleVO") Integer articleNO) {
		logger.info("findArticle 메서드 호출");
		ArticleVO vo = new ArticleVO();
		vo.setArticleNO(articleNO);
		vo.setWriter("홍길동");
		vo.setTitle("안녕하세요");
		vo.setContent("홀길동 글입니다.");
	...
	public ResponseEntity<String> modArticle (@PathVariable("articleNO") Integer articleNO, @RequestBody ArticleVO articleVO) {
		...
		try {
			...
			logger.info(articleVO.toString());
			resEntity = new ResponseEntity("REMOVE_SUCCEEDE" , HttpStatus.OK);
		}catch(Exception e) {
			resEntity = new ResponseEntity(e.getMessage(), HttpStatus.OK);
		}
